...
start "Middleware" cmd /k "cd D:\Works\Oppen_Web_UI_fresh\llm-mw && D:\Works\.venv\Scripts\activate.bat && set MW_SECRET=test-secret-key-for-development-only-change-in-production && set LITELLM_BASE=http://127.0.0.1:4000/v1 && set LITELLM_KEY=sk-1234 && set ADMIN_KEY=admin_master_key_456 && python -m uvicorn main:app --host 0.0.0.0 --port 5000"
timeout /t 5 /nobreak >nul

REM Start OpenWebUI in new window
echo [3/3] Starting OpenWebUI (Port 3000)...
start "OpenWebUI" cmd /k "cd D:\Works\Oppen_Web_UI_fresh && D:\Works\.venv\Scripts\activate.bat && set DATA_DIR=D:\Works\Oppen_Web_UI_fresh\openwebui_data && set WEBUI_SECRET_KEY=your-secret-key-here && set LITELLM_API_KEY=subkey_admin_123 && open-webui serve --port 3000"
