...
curl -s http://localhost:5000/v1/models -H "Authorization: Bearer subkey_admin_123"
echo.
echo.

echo [3/7] Testing /admin/usage endpoint...
curl -s http://localhost:5000/admin/usage -H "Authorization: Bearer admin_master_key_456"
echo.
echo.

echo [4/7] Testing /v1/_mw/summary endpoint...
curl -s "http://localhost:5000/v1/_mw/summary?minutes=60" -H "Authorization: Bearer admin_master_key_456"
echo.
echo.
