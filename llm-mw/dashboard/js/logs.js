...
// Unique users and models for dropdown
...
export async function loadLogs(append = false) {
    try {
        ...
        if (append) {
            ...
            // CRITICAL FIX: Clear dropdown data when loading fresh (not appending)
            // This ensures dropdowns match filtered data
            uniqueUsers.clear();
            uniqueModels.clear();
        }
        ...
        // Populate unique users and models for dropdowns from current filtered data
...
function updateDropdownOptions() {
    ...
    const userOptions = Array.from(uniqueUsers).sort().map(u => 
    ...
    const modelOptions = Array.from(uniqueModels).sort().map(m => 
...
export function resetLogFilters() {
    ...
    // CRITICAL FIX: Clear dropdown data to rebuild from fresh unfiltered data
    uniqueUsers.clear();
    uniqueModels.clear();
