...
| Middleware Subkey | YOUR_SUBKEY_ADMIN |
| LiteLLM Master Key | YOUR_LITELLM_KEY |
...
Invoke-RestMethod -Uri "http://localhost:5000/v1/chat/completions" `
    -Method POST `
    -Headers @{"Authorization"="Bearer YOUR_SUBKEY_ADMIN"; "Content-Type"="application/json"} `
    -Body $body
```
...
# Models list
curl -H "Authorization: Bearer YOUR_SUBKEY_ADMIN" http://localhost:5000/v1/models
```
