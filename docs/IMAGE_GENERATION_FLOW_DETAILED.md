...
IMAGES_OPENAI_API_KEY=subkey_admin_123               # Subkey, KHÔNG phải OpenAI key!
...
def require_user(request: Request) -> Dict[str, Any]:
    ...
    subkey = auth.split(" ", 1)[1].strip()  # "subkey_admin_123"
...
curl -H "X-Admin-Key: admin_master_key_456" \
  http://localhost:5000/admin/usage | jq '.[] | select(.user_id=="admin") | .quota'

# Reset quota for user
curl -X POST -H "X-Admin-Key: admin_master_key_456" \
  -H "Content-Type: application/json" \
