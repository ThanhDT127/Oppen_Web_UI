...
| 03  | **Rate limit**  | ❌ Không có                          | ✅ 10 req/s chat, 5 req/phút login |
...
Firewall công ty (do IT quản lý)
    │  NAT: port 51122 → 10.0.0.1:3000
    ▼
┌────────────────────────────────────────────────────────┐
│  Server 10.0.0.1 (20 CPU / 32GB RAM)              │
│                                                        │
│  ┌── Nginx (:3000 HTTPS) ──────────────────────────┐   │
...
| --- | ------- | ---------- | ----- | ---------------- | ----------------- |
| 01  | `chat`  | 10 req/s   | 50    | Trang chính `/`  | Chống spam chat   |
| 02  | `login` | 5 req/phút | 3     | `/api/v1/auths/` | Chống brute force |

### 3.3 Timeout

| STT | Tham số                 | Giá trị | Lý do                                       |
| --- | ----------------------- | ------- | ------------------------------------------- |
| 01  | `proxy_read_timeout`    | 300s    | LLM response có thể mất 1-2 phút (câu dài)  |
| 02  | `proxy_send_timeout`    | 300s    | Upload file lớn cần thời gian               |
| 03  | `proxy_connect_timeout` | 10s     | Kết nối backend nhanh (cùng Docker network) |
...
Cert **wildcard `*.example.com`** do công ty mua, IT (anh Huy) cung cấp.

### File cert

| STT | File            | Nguồn                                            | Vị trí                    |
| --- | --------------- | ------------------------------------------------ | ------------------------- |
| 01  | `fullchain.pem` | Gộp từ `STAR.example.com.crt` + `.ca-bundle` | `nginx/ssl/fullchain.pem` |
| 02  | `privkey.pem`   | Copy từ `STAR.example.com_key.txt`           | `nginx/ssl/privkey.pem`   |
...
| 02  | `/static/*`       | `/static/`       | WebUI :8080 | ❌ Không    | Hình ảnh, fonts           |
| 03  | `/ws/*`           | `/ws/`           | WebUI :8080 | ❌ Không    | WebSocket (timeout 24h)   |
| 04  | `/api/v1/auths/*` | `/api/v1/auths/` | WebUI :8080 | 5 req/phút | Login (chống brute force) |
| 05  | `/dashboard*`     | `/dashboard`     | MW :5000    | ❌ Không    | Admin dashboard           |
| 06  | `/v1/_mw/*`       | `/v1/_mw/`       | MW :5000    | 10 req/s   | MW API + SSE stream       |
| 07  | `/v1/*`           | `/v1/`           | MW :5000    | ❌ Không    | LLM chat API (streaming)  |
| 08  | `/*`              | `/`              | WebUI :8080 | 10 req/s   | Trang chính, catch-all    |
