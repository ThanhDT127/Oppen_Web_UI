...
| **SSL cert** | Wildcard `*.example.com` (công ty cung cấp) |
...
| File | Nguồn | Vị trí |
|------|-------|--------|
| `fullchain.pem` | Gộp từ `STAR.example.com.crt` + `.ca-bundle` | `nginx/ssl/fullchain.pem` |
| `privkey.pem` | Copy từ `STAR.example.com_key.txt` | `nginx/ssl/privkey.pem` |
