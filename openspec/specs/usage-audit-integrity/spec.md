# usage-audit-integrity Specification

## Purpose
Guarantee the integrity of usage audit data in `mw_audit_log`: every proxied LLM request produces exactly one audit row with complete identity fields (user, model, endpoint) and correctly priced cost, so dashboard aggregations (TopModel, TopUsers, totals) never show synthetic "unknown" entries or double-counted spend.
## Requirements
